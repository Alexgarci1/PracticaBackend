...
final class PersonRelationsController extends ElementRelationsBaseController
{
    ...
    public function getProducts(Request $request, Response $response, array $args): Response
    {
        ...
        $idProduct      = (int) $args[static::getEntityIdName()];
        $personProduct  = $this->entityManager
            ->getRepository(static::getEntityClassName())
            ->find($idProduct);
        $itemsProducts   = $personProduct instanceof Person
            ? $personProduct->getProducts()->toArray()
            : [];
